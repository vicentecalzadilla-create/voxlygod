...
const LoginPage = () => {
  ...
  const handleEmail = async (event: React.FormEvent) => {
    ...
    if (!email.trim() || password.length < 6) {
      toast({ title: 'Datos incompletos', description: 'Ingresa un email y una contraseña de al menos 6 caracteres.', variant: 'destructive' });
      return;
    }

    setLoading(true);
    const result = isSignUp
      ? await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${window.location.origin}/create` } })
    ...
    if (isSignUp && !result.data.session) {
      toast({ title: 'Revisa tu correo', description: 'Confirma tu cuenta antes de publicar audios.' });
      return;
    }
      ...
      <div className="relative w-full max-w-sm mx-auto px-6 pb-12 space-y-8" style={{ animation: 'fade-in-up 0.8s ease-out' }}>
        ...
        <form onSubmit={handleEmail} className="space-y-3">
          ...
          >
            ...
            {isSignUp ? 'Crear cuenta con Email' : 'Entrar con Email'}
          </button>
